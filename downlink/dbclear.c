...
static void DbaseClearNormal(void)
{
	...
	while(NULL != fgets(filename, 95, pf)) {
		...
		if(2 == rtn) { //month
			//清除6个月以前的月数据
			if(filemon > curmon || (curmon-filemon) > 6) {
				RemoveFile(filename);
			}
		}
		else { //day
			//清除2个月以前的天数据 
			//广标要求保存60天的日数据。modified by yangzhilong. 2010.5.14
			if(filemon > curmon || (curmon-filemon) > 2) {
				RemoveFile(filename);
			}
			else if((curmon - filemon) == 2) {
				if(curclock.day -fileclock.day >=1) {
					RemoveFile(filename);
				}
				/*
